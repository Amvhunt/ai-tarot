...
DECKS = [
  ...
  {
    "id": 1,
    "style": f'Anime futuristic style',
    "cards": [
      {
        "id": j,
        "name": CARDS[j]['name'],
  ...
  {
    "id": 2,
    "style": f'Digital painting in modern realistic style, modernity 2025, white skin, luxury, professional makeup.  Natural lighting, with soft painterly textures, office places',
    "cards": [
      {
        "id": j,
        "name": CARDS[j]['name'],
...
def generate_pages():
  ...
  with app.app_context():
    index_html = render_template('index.html', decks=DECKS)
    index_html = index_html.replace("http://localhost:5000/", "")
    with open('index.html', 'w', encoding='utf-8') as f:
      f.write(index_html)

    for deck in DECKS:
      deck_path = os.path.join('deck', str(deck['id']))
      os.makedirs(deck_path, exist_ok=True)
      deck_html = render_template('deck.html', deck=deck, cards=CARDS)
      deck_html = deck_html.replace("http://localhost:5000/", "../../")
      with open(os.path.join(deck_path, 'index.html'), 'w', encoding='utf-8') as f:
        f.write(deck_html)
